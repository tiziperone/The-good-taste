...
@section('titulo', 'The Good Taste - Bondiolas')
    ...
    <div class="col-12 col-md-5 col-lg-4">
      <div class="card text-bg-dark border-warning shadow-sm h-100 position-relative">
        ...
        <div class="card-body d-flex flex-column">
          ...
          <h4 class="fw-bold mb-3">${{ number_format($bondiola->precio, 0, ',', '.') }}</h4>
          <div class="mt-auto">
            @if($bondiola->stock > 0)
            @auth
            <button type="button" class="btn btn-warning fw-bold text-dark btn-comprar-ahora" data-id="{{ $bondiola->id }}">Comprar</button>
            <button type="button" class="btn btn-outline-light ms-2 btn-agregar-carrito" data-id="{{ $bondiola->id }}">Agregar <i class="bi bi-cart"></i></button>
            @else
