...
@section('titulo', 'The Good Taste - Gestión de Consultas')
    ...
    <div class="row">
        ...
        <div class="col-md-9 col-lg-10">
                    ...
                    <table class="table table-dark table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ $c->estado ? 'opacity-50' : '' }}">
                                <td class="ps-3">
                                    @if($c->users_id)
                                    <span class="badge bg-primary mb-1"><i class="bi bi-person-check-fill"></i> Registrado</span>
                                    <div class="fw-bold">{{ $c->user->name }}</div>
                                    @else
                                    <span class="badge bg-secondary mb-1"><i class="bi bi-person-x-fill"></i> Invitado</span>
                                ...
                                <td class="text-center" style="min-width: 250px;">
                                    <form action="{{ route('consultas.responder', $c->id) }}" method="POST">
                                        @csrf
                                        <textarea name="respuesta" class="form-control bg-secondary text-white border-0 mb-2" rows="2" placeholder="Escribe tu respuesta aquí...">{{ $c->respuesta }}</textarea>
                                        <div class="d-flex justify-content-between">
                                            <button type="submit" class="btn btn-sm btn-primary"><i class="bi bi-send"></i> Enviar</button>
                                            <button formaction="{{ route('consultas.marcarLeido', $c->id) }}" type="submit" class="btn btn-sm {{ $c->estado ? 'btn-success' : 'btn-warning' }}">
                                                <i class="bi {{ $c->estado ? 'bi-check-all' : 'bi-clock' }}"></i>
                                            </button>
                                        </div>
                                    </form>
                                    <form action="{{ route('consultas.eliminar', $c->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar esta consulta?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger mt-2 w-100"><i class="bi bi-trash"></i> Eliminar</button>
                                    </form>
                                </td>
                            </tr>
                            @empty
            ...
            <div class="d-flex justify-content-center mt-3">
                {{ $consultas->links('pagination::bootstrap-5') }}
            </div>
            @endif
        </div>
    </div>
